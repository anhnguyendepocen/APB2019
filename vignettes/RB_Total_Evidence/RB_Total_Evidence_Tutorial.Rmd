...
heterogeneity across sites (the and in Fig. 1). This
...
according to an exponential density (Fig. 5). This is the
for the ([[*i.e.,*]{}]{}Fig. 1) that we will use in this
...
![](figures/tikz/uexp_gm-eps-converted-to.png)
...
Just like with the molecular data (section Intro to GTR and UExp),
...
1. The relaxed clock model we described for the
molecular data in section Intro to GTR and UExp it allows the
...
![](figures/tikz/morph_clock_gm-eps-converted-to.png)
...
this analysis are listed in Table 1 along with the age
...
This part of the workshop will take place in RevBayes.

In my course's vignettes directory, find RB_Total_Evidence. Change directories into it.
...
the main directory, [[`RB_TotalEvidenceDating_FBD_Tutorial`]{}]{}, thus, if you are using a Unix-based operating system, we recommend that you add the [`RevBayes`]{}binary to your path.
...
function.
```{rev, engine.path = '/Users/april/software/rb', engine.opts=list(append=TRUE)}
...
average of the age range for fossil species (see the table of ages above).
...
Data Files, we mentioned that the two data matrices
...
Fig. 3) to 1. The parameter [[`rho`]{}]{} will be
...
constraints we applied (section FBD - Constraints).
...
(described in section GTR & UExp  and shown in
Fig. 5), we first define the mean branch rate as an
...
As stated in the introduction (section Morphology) we will
...
an exponential distribution (section Morphological Clock).
...
only variable characters (discussed in Intro to Morphology).
